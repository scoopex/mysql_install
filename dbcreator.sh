...
DROP USER IF EXISTS '${DBUSER_PREFIX}_app'@'%';
DROP USER IF EXISTS '${DBUSER_PREFIX}_adm'@'%';
DROP USER IF EXISTS '${DBUSER_PREFIX}_app'@'localhost';
DROP USER IF EXISTS '${DBUSER_PREFIX}_adm'@'localhost';
EOF
)"
fi
...
CREATE USER '${DBUSER_PREFIX}_app'@'%' IDENTIFIED BY '${APP_PASSWORD}' WITH MAX_USER_CONNECTIONS $MAX_CONN_APP ;
CREATE USER '${DBUSER_PREFIX}_app'@'localhost' IDENTIFIED BY '${APP_PASSWORD}' WITH MAX_USER_CONNECTIONS $MAX_CONN_APP ;
REVOKE ALL PRIVILEGES ON *  . *  FROM '${DBUSER_PREFIX}_app'@'%';
REVOKE ALL PRIVILEGES ON *  . *  FROM '${DBUSER_PREFIX}_app'@'localhost';
CREATE USER '${DBUSER_PREFIX}_adm'@'%' IDENTIFIED BY '${ADM_PASSWORD}' WITH MAX_USER_CONNECTIONS $MAX_CONN_ADM;
CREATE USER '${DBUSER_PREFIX}_adm'@'localhost' IDENTIFIED BY '${ADM_PASSWORD}' WITH MAX_USER_CONNECTIONS $MAX_CONN_ADM;
...
GRANT SELECT , INSERT , UPDATE , DELETE , EXECUTE ON \`$db\` . * TO '${DBUSER_PREFIX}_app'@'%';
GRANT SELECT , INSERT , UPDATE , DELETE , EXECUTE ON \`$db\` . * TO '${DBUSER_PREFIX}_app'@'localhost';

GRANT SELECT , INSERT , UPDATE , DELETE , CREATE , DROP , INDEX , ALTER , CREATE TEMPORARY TABLES , LOCK TABLES , REFERENCES,
 CREATE VIEW , SHOW VIEW , CREATE ROUTINE, ALTER ROUTINE, EXECUTE, EVENT, TRIGGER ON \`$db\` . * TO '${DBUSER_PREFIX}_adm'@'%';
GRANT SELECT , INSERT , UPDATE , DELETE , CREATE , DROP , INDEX , ALTER , CREATE TEMPORARY TABLES , LOCK TABLES , REFERENCES,
 CREATE VIEW , SHOW VIEW , CREATE ROUTINE, ALTER ROUTINE, EXECUTE, EVENT, TRIGGER
 ON \`$db\` . * TO '${DBUSER_PREFIX}_adm'@'localhost';
EOF
)"
done
...
if [ "$TEST" != "true" ];then
 echo "$COMMANDS" | mysql $MYSQL_ARGS -vv mysql 
 if [ "$?" != "0" ];then
   echo "FAILED"
   echo
   echo "Kommandos:"
